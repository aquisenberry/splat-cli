...
var cordovaExecutablePath = path.resolve(
  __dirname,
...
if (!appMetadata) {
  console.error(
    "Your package.json does not have a properly formatted 'appMetadata' property. Unable to package cordova app(s)"
  );
} else {
  runChildProcess("npm run build")
    .then(() =>
      createCordovaProject(
        cordovaExecutablePath,
...
function runChildProcess(
  command,
  failureMessage = "Failed to Run Child Process"
) {
  return new Promise((resolve, reject) => {
    ...
    childProcess.on("close", function (code, signal) {
      if (code === 0) {
        ...
        reject(
          new Error(`${failureMessage} Result Code ${code}. Signal ${signal}`)
        );
      }
    });
  });
